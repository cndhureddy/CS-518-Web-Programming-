...
include("connect.php");
function retrieving_messages_user($conn,$to_func_user_id,$temp_user_id){
    //echo $to_func_user_id;
   // echo $temp_user_id;
...
//    $query_message_id="select message_id,reaction,count(*) from message_reaction group by message_id,reaction ";
   ...
   // $row_channel_status=$result_channel_status->fetch_array(MYSQLI_ASSOC);
    ...
    while($row=$result->fetch_array(MYSQLI_ASSOC))
    {
        ...
        // echo $row["timestamp"];
        //$message_id_for_likes=$row["dr_message_id"];
        ...
        if($temp_time==$formated_time_am_pm and $row["from_user_id"]==$temp_user_id) {
            ...
            */
            ...
            if($row["message_type"]=="message") {
                echo "<div class=\"message_display_sub the_whole_message_sub\" > <div class=\"message_sub\" id=\"" . htmlspecialchars($row["dr_message_id"]) . "_div\">" . htmlspecialchars($row["message"]) . " <br> ";
            }
            if($row["message_type"]=="codesnip"){

                echo "<div class=\"message_display_sub the_whole_message_sub\" > <div class=\"message_sub\" id=\"" . htmlspecialchars($row["dr_message_id"]) . "_div\"><pre><code>" . htmlspecialchars($row["message"]) . " </code></pre><br> ";
                // echo "<div class=\"clear\"></div>";
            }
            if($row["message_type"]=="image_link" ){

                echo "<div class=\"message_display_sub the_whole_message_sub\" > <div class=\"message_sub\" id=\"" . htmlspecialchars($row["dr_message_id"]) . "_div\"><a href=\"" . htmlspecialchars($row["message"]) ." \">" . htmlspecialchars($row["message"]) . "</a><br><img height=\"200\" width=\"200\" src=\"" . htmlspecialchars($row["message"]) . "\"\><br> ";
                //echo "<div class=\"clear\"></div>";

            }
            if($row["message_type"]=="picture"){

                echo "<div class=\"message_display_sub the_whole_message_sub\" > <div class=\"message_sub\" id=\"" . htmlspecialchars($row["dr_message_id"]) . "_div\"><img height=\"200\" width=\"200\" src=\"" . htmlspecialchars($row["message"]) . "\"\><br> ";
                //echo "<div class=\"clear\"></div>";
           ...
           // $query_thread_count="select count(*) from thread where message_id='".$row["dr_message_id"]."'";
            ...
            //if($count_like>0)
            //{
            //    echo "<i class=\"fa fa-thumbs-o-up\" aria-hidden=\"true\" id=\"". htmlspecialchars($row["dr_message_id"]). "_like\">".$count_like."</i> &nbsp;";
           // }
            //if($count_dislike>0)
            //{
              //  echo   "<i class=\"fa fa-thumbs-o-down\" aria-hidden=\"true\"  id=\"". htmlspecialchars($row["dr_message_id"]). "_dislike\"  >".$count_dislike."</i>&nbsp;";
            //}
           // echo "</div>";
            //echo "<div class=\"thread_count_div_".htmlspecialchars($row["dr_message_id"])."\">";
            //if($row_count["count(*)"]>0)
            //{
              //  echo "<div class=\"unique_count_".htmlspecialchars($row["dr_message_id"])."  \">";
                //if($row_channel_status["archieved_status"]=="unarchieved") {
                 //   echo "<button id=\"thread_count_" . htmlspecialchars($row["dr_message_id"]) . "\"   value=\" " . htmlspecialchars($row["message_id"]) . "\"   >" . $row_count["count(*)"] . " replies </button>";
                //}
                //else{

                    //echo "<button archieved=\"yes\" id=\"thread_count_" . htmlspecialchars($row["dr_message_id"]) . "\" value=\" " . htmlspecialchars($row["message_id"]) . "\"    >" . $row_count["count(*)"] . " replies</button>";
  ...
  */
            ...
            echo "<div class=\"message_user_full_name\"><span class=\"fullname_msg_span\" \>" . $row_user['full_name'] . " </span>" . $formated_time_am_pm . "</div>";
            if($row["message_type"]=="message") {
                echo "<div class=\"message_display \" id=\"" . htmlspecialchars($row["dr_message_id"]) . "_div\"><div class=\"only_message\">" . htmlspecialchars($row["message"]) . "  </div>  ";
            }
            if($row["message_type"]=="picture" ){

                echo "<div class=\"message_display \" id=\"" . htmlspecialchars($row["dr_message_id"]) . "_div\"><div class=\"only_message\"><img height=\"200\" width=\"200\" src=\"" . htmlspecialchars($row["message"]) . "\"\>  </div>  ";
                // echo "<div class=\"clear\"></div>";
            }
            if($row["message_type"]=="image_link"){


                echo "<div class=\"message_display \" id=\"" . htmlspecialchars($row["dr_message_id"]) . "_div\"><div class=\"only_message\"><a href=\"" . htmlspecialchars($row["message"]) ." \">" . htmlspecialchars($row["message"]) . "</a><br><img height=\"200\" width=\"200\" src=\"" . htmlspecialchars($row["message"]) . "\"\>  </div>  ";
            ...
            if($row["message_type"]=="codesnip"){
                ...
                echo "<div class=\"message_display \" id=\"" . htmlspecialchars($row["dr_message_id"]) . "_div\"><div class=\"only_message\"><a href=\"" . htmlspecialchars($row["message"]) ." \"download>" . htmlspecialchars($row["message"]) . "</a><br>  </div>  ";
           ...
           /* $query_thread_count="select count(*) from thread where message_id='".$row["dr_message_id"]."'";
            ...
            if($row_count["count(*)"]>0) {
                echo "<div class=\"unique_count_".htmlspecialchars($row["dr_message_id"])."\">";
                if($row_channel_status["archieved_status"]=="unarchieved") {
                    echo " <button id=\"thread_count_" . htmlspecialchars($row["dr_message_id"]) . "\" value=\" " . htmlspecialchars($row["dr_message_id"]) . "\"    >" . $row_count["count(*)"] . " replies</button>";
                }else{
                    echo " <button archieved=\"yes\" id=\"thread_count_" . htmlspecialchars($row["dr_message_id"]) . "\" value=\" " . htmlspecialchars($row["dr_message_id"]) . "\"    >" . $row_count["count(*)"] . " replies</button>";
            ...
            echo "</div>";
            if($row_channel_status["archieved_status"]=="unarchieved") {
              //  echo "<div class=\"message_reactions_with_user col-md-2\"><button id=\"like\"  class=\"like_dislike\" value=\"" . htmlspecialchars($row["dr_message_id"]) . "\"><i class=\"fa fa-thumbs-o-up\" aria-hidden=\"true\"></i></button><button id=\"dis_like\"   class=\"like_dislike\"  value=\"" . htmlspecialchars($row["message_id"]) . "\"> <i class=\"fa fa-thumbs-o-down\" aria-hidden=\"true\"></i></button> <button id=\"thread_message\"   class=\"like_dislike\"  value=\" " . htmlspecialchars($row["dr_message_id"]) . "\"> <i class=\"fa fa-reply\" aria-hidden=\"true\"></i></button> </div>";
            }
           // echo "</div>";
        }
    }
}
retrieving_messages_user($conn,$to_func_user_id,$temp_user_id);
mysqli_close($conn);
