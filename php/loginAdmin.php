...
if($email!=""&$password!="")
{
    $query = mysqli_query($conn,"select * from admin where email_id='".mysqli_real_escape_string($conn,$email)."' and password='".mysqli_real_escape_string($conn,$password)."'");
    ...
    if($res)
    {
       ...
       // $query_workspace = mysqli_query($conn,  "select work_space.work_space_name from work_space,users where users.email_id='".mysqli_real_escape_string($conn,$email)."' and work_space.work_space_url = users.work_space_url");
        ...
        mysqli_close($conn);
        $parameter="channel_id="."1"."&user_id="."18"."&channel_name=general";
